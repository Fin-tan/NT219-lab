...
// Hàm nhập plaintext từ file
void InputPlainTextFromFile(string& plain) {
    wstring filePath;
    wcout << L"<1> Nhập đường dẫn file chứa plaintext: ";
    fflush(stdin);
    getline(wcin, filePath);

    string sFilePath = WStringToString(filePath);
    try {
        FileSource fs(sFilePath.c_str(), true, new StringSink(plain));
...
// Cập nhật phương thức nhập plaintext từ màn hình hoặc file
void InputPlainText(string& plain) {
    wcout << L"<1> Chọn phương thức nhập plaintext:\n";
    wcout << L"\t[1] Nhập từ màn hình\n";
    wcout << L"\t[2] Nhập từ file\n";
    int choice;
    wcout << L"\tLựa chọn: ";
    wcin >> choice;
    wcin.ignore(1);
    ...
    switch (choice) {
        ...
        case 2:
            ...
            break;
        default:
            wcout << L"Lựa chọn không hợp lệ. Vui lòng thử lại.\n";
