...
void ModuleEncryption()
{
    try
    {
        ...
        wcout << L"* Time to ecrypt: "<< duration.count()/10000 <<L" microseconds"<< endl;
...
void ModeleDecryption()
{
    try
    {
        ...
        auto stop = high_resolution_clock::now();
        auto duration = duration_cast<microseconds>(stop - start);  
        wcout << L"* Time to decrypt: "<< duration.count()/10000 <<L" microseconds"<< endl;
...
void InputPlaintextFromFile(string& plain)
{
    FileSource file("plain300.txt", true, new StringSink(plain));
